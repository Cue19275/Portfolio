...
void Timer0IntHandler(void) {
    ...
    switch((int)filtro[10]){ //caso para poder variar la frecuencia de muestreo
    case 0:
        freq=0;
    ...
    //************************filtro******************************************
    y1 = filtro[0]*pui32ADC0Value[0]+filtro[1]*x1_n_1+filtro[2]*x1_n_2+filtro[3]*x1_n_3+filtro[4]*x1_n_4
             -filtro[6]*y1_n_1-filtro[7]*y1_n_2-filtro[8]*y1_n_3-filtro[9]*y1_n_4;
    ...
    x1_n_3 = x1_n_2;
    x1_n_2 = x1_n_1;
    x1_n_1 = pui32ADC0Value[0];
    UARTprintf("%d\n",pui32ADC0Value[0]); //envio de caracteres de la tiva
    // Display the AIN0 (PE3) digital value on the console.
    //*****************************DAC*********************************************
.
    //Colocar los limites para que el DAC los pueda procesaar apropiadamente
    if (y1 < 0){
        temp = 0;
    }else if (y1>4095){
        temp = 4095;
    ...
    //se envia el dato del filtro al DAC para mostrarlo en el osciloscopio
    pui32DataTx[0] = (uint32_t)((0b0111 << 12) | (0x0FFF & (int)temp));
...
void UARTIntHandler(void){
    ...
    while(UARTCharsAvail(UART0_BASE)){
        ...
        //
        //se lee el string recibidio desde matlab
        data = UARTgets(buffer,100);
        //
    ...
    //se separa el strins en cada coeficiente para la ecuacion de diferencias
    ...
    toks = strtok(NULL, ",");
    filtro[11] = atof(toks);
    //limpieza de variables de la ecuacion de diferencias
    y1 = 0;
    y1_n_1 = 0;
    y1_n_2 = 0;
    y1_n_3 = 0;
